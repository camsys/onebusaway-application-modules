		...
		for (AgencyWithCoverageBean agency : _transitDataService.getAgenciesWithCoverage()) {
			for (RouteBean routeBean : _transitDataService.getRoutesForAgencyId(agency.getAgency().getId()).getList()) {
				...
				if (routeShortName == null)
					routeShortName = routeBean.getLongName();
				if (routeShortName == null)
					routeShortName = routeBean.getId();
				String routeLongName = routeBean.getLongName();
				if (routeLongName == null)
					routeLongName = routeBean.getShortName();
				if (routeLongName == null)
					routeLongName = routeBean.getId();
				
				_routeShortNameToRouteBeanMap.put(routeShortName.toUpperCase(), routeBean);
				_routeLongNameToRouteBeanMap.put(routeLongName, routeBean);
			}
		}
